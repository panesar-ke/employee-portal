import { pgTable, pgEnum, serial, varchar, integer, foreignKey, text, uuid, timestamp, index, boolean, bigint, unique, date, numeric, uniqueIndex, primaryKey } from "drizzle-orm/pg-core"
...
export const userRoleEnum = pgEnum("userRoleEnum", ['SUPER ADMIN', 'ADMIN', 'STANDARD USER'])
export const discountTypeEnum = pgEnum("discountTypeEnum", ['AMOUNT', 'PERCENTAGE', 'NONE'])
export const vatTypeEnum = pgEnum("vatTypeEnum", ['INCLUSIVE', 'EXCLUSIVE', 'NONE'])
export const stockMovementTypeEnum = pgEnum("stockMovementTypeEnum", ['OPENING_BAL', 'CONVERSION_OUT', 'CONVERSION_IN', 'CONVERSION', 'TRANSFER', 'ISSUE', 'GRN'])
export const employeeCategory = pgEnum("employeeCategory", ['NON-UNIONISABLE', 'MANAGEMENT', 'UNIONISABLE'])
export const employeeStatus = pgEnum("employeeStatus", ['RETIRED', 'RESIGNED', 'TERMINATED', 'ACTIVE'])
export const employmentType = pgEnum("employmentType", ['NO CONTRACT', 'INTERN', 'CASUAL', 'CONTRACT', 'PERMANENT'])
export const genderEnum = pgEnum("genderEnum", ['FEMALE', 'MALE'])
export const maritalStatusEnum = pgEnum("maritalStatusEnum", ['WIDOWED', 'DIVORCED', 'MARRIED', 'SINGLE'])
...
export const ordersHeader = pgTable("orders_header", {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	id: bigint("id", { mode: "number" }).primaryKey().notNull(),
	reference: text("reference").notNull(),
	...
	mrqId: integer("mrq_id").references(() => mrqHeaders.id, { onDelete: "cascade" } ),
...
(table) => {
	return {
		ordersHeaderReferenceUnique: unique("orders_header_reference_unique").on(table.reference),
	}
});

export const grnsDetails = pgTable("grns_details", {
	id: uuid("id").defaultRandom().notNull(),
	headerId: integer("header_id").notNull().references(() => grnsHeader.id),
...
export const materialIssuesHeader = pgTable("material_issues_header", {
	id: uuid("id").defaultRandom().primaryKey().notNull(),
...
(table) => {
	return {
		materialIssuesHeaderIssueNoUnique: unique("material_issues_header_issue_no_unique").on(table.issueNo),
	}
});
...
export const conversions = pgTable("conversions", {
	...
	convertingItem: uuid("converting_item").notNull().references(() => products.id, { onDelete: "cascade" } ),
	convertingQuantity: numeric("converting_quantity").notNull(),
	convertedItem: uuid("converted_item").notNull().references(() => products.id, { onDelete: "cascade" } ),
...
export const mrqDetails = pgTable("mrq_details", {
	id: uuid("id").defaultRandom().primaryKey().notNull(),
	headerId: integer("header_id").notNull().references(() => mrqHeaders.id, { onDelete: "cascade" } ),
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	requestId: bigint("request_id", { mode: "number" }).notNull(),
	projectId: uuid("project_id").notNull().references(() => projects.id, { onDelete: "cascade" } ),
	itemId: uuid("item_id").references(() => products.id, { onDelete: "cascade" } ),
	unitId: integer("unit_id").notNull().references(() => uoms.id, { onDelete: "cascade" } ),
	qty: numeric("qty").notNull(),
...
(table) => {
	return {
		mrqDetailsRequestIdUnique: unique("mrq_details_request_id_unique").on(table.requestId),
	}
});
...
export const tempDebtors = pgTable("temp_debtors", {
	id: text("id").primaryKey().notNull(),
...
(table) => {
	return {
		tempDebtorsEmailUnique: unique("temp_debtors_email_unique").on(table.email),
		tempDebtorsContactUnique: unique("temp_debtors_contact_unique").on(table.contact),
	}
});
...
export const employees = pgTable("employees", {
	...
	department: integer("department").notNull().references(() => departments.id, { onDelete: "restrict" } ),
	designation: integer("designation").notNull().references(() => designations.id, { onDelete: "restrict" } ),
	...
	employeeStatus: employeeStatus("employee_status").default('ACTIVE').notNull(),
...
(table) => {
	return {
		...
		employeeOthernamesIdx: index("employee_othernames_idx").on(table.otherNames),
	}
});
...
export const employeesContacts = pgTable("employees_contacts", {
	...
	countyId: integer("county_id").references(() => counties.id, { onDelete: "restrict" } ),
...
export const ordersDetails = pgTable("orders_details", {
	id: uuid("id").defaultRandom().primaryKey().notNull(),
	headerId: integer("header_id").notNull().references(() => ordersHeader.id, { onDelete: "cascade" } ),
	projectId: uuid("project_id").notNull().references(() => projects.id, { onDelete: "cascade" } ),
	itemId: uuid("item_id").references(() => products.id, { onDelete: "cascade" } ),
	qty: numeric("qty").notNull(),
	rate: numeric("rate").notNull(),
...
(table) => {
	return {
		userRolesRoleIdUserIdPk: primaryKey({ columns: [table.userId, table.roleId], name: "user_roles_role_id_user_id_pk"})
	}
});
...
(table) => {
	return {
		accountsProviderProviderAccountIdPk: primaryKey({ columns: [table.provider, table.providerAccountId], name: "accounts_provider_provider_account_id_pk"})
	}
});